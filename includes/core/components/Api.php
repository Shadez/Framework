...
class Api_Component extends Component
{
	...
	{
		if ($this->c('Config')->getValue('site.api.disabled'))
		{
	...
	{
		$this->m_apiResponse = array(
			'action' => $this->c('Core')->getRawUrl($data['idx'])
		);
	}
	...
	protected function apiMethod_coregetversion($method, $data)
	{
		...
		if ($data['fullVersion'])
			$this->m_apiResponse['version'] = '0.43.1;0.1';
		else
			$this->m_apiResponse['version'] = '0.4';

		if ($data['info'])
			$this->m_apiResponse['info'] = 'Shadez Framework: Core Version 0.43.1, API Version 0.1';
		else
