...
class Events_Component extends Component
{
	...
	protected function isCanBeTriggered($event)
	{
		...
		{
			$this->c('Log')->writeDebug('%s : event "%s" can not be triggered: event was not found!', __METHOD__, $event);
			return false;
		}
		if ($this->m_events[$event]['disabled'])
		{
			$this->c('Log')->writeDebug('%s : event "%s" can not be triggered: event was disabled!', __METHOD__, $event);
			return false;
		}
		if (!is_callable($this->m_events[$event]['callback']))
		{
			$this->c('Log')->writeDebug('%s : event "%s" can not be triggered: event callback is not callable!', __METHOD__, $event);
			return false;
		}
